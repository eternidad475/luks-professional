...
- 画像は用途で 2 つに分かれています:
  - **① 評価用画像（シーケンス）**: マクロ／ミクロそれぞれ**複数枚を順番に追加**できます。サムネイルでプレビューでき、その並び順でモーフィング動画を生成・**ダウンロード**できます（審美評価は先頭画像で実施）
  - **② シミュレーション用画像（術後イメージ・任意）**: マクロ／ミクロ。「評価用の最後の画像 → この画像」のモーフィング動画を生成・ダウンロードできます
- 生成された動画はレポート内の各動画の「⬇ ダウンロード」リンクから保存できます
- 各スロットの「＋ 画像を追加」を押すと、端末標準の選択メニュー（写真を撮る／写真ライブラリ／ファイルを選択）が直接開きます（アプリ独自ポップアップは出しません＝二重表示を避けるため）
- 「解析してレポート生成」で評価・推奨治療・シミュレーション動画を生成し、結果ページへ遷移

> スマホの「写真を撮る」は端末標準カメラを使用します（OS のセキュリティ仕様のため、Web ページ側で撮影ガイド線を重ねることはできません）。
