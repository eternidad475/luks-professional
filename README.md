...
- マクロ術前/術後・ミクロ術前/術後の各スロットの「＋ 画像を追加」を押すと、ポップアップで「📷 カメラで撮影（白ガイド付き）」または「🖼 写真・ファイルから選択」を選べます
- 「カメラで撮影」はブラウザの `getUserMedia` で白ガイドを重ねたライブ撮影、「写真・ファイルから選択」は端末標準のピッカー（写真ライブラリ／ファイル）に委譲します
- ※ iOS/Android では `<input type="file">` を開くと OS が独自の選択メニュー（写真ライブラリ／写真を撮る／ファイル選択）を表示します。アプリ側で写真/ファイルを別ボタンに分けると OS メニューと二重になるため、1 つの「写真・ファイルから選択」に集約しています
- **撮影ガイド**: 「カメラで撮影」では Invisalign 系の撮影アプリのように、構図合わせ用の**白いガイドグリッド**を表示します（マクロ=顔の正中線・瞳孔線・口唇ライン・顔の輪郭、ミクロ=歯列の正中線・咬合平面・スマイルアーチ・枠）。ガイドは整列補助のオーバーレイで、撮影画像には写り込みません
