...
- マクロ術前/術後・ミクロ術前/術後の各スロットの「＋ 画像を追加」を押すと、ポップアップで「📷 カメラで撮影」「🖼 写真ライブラリから選択」「⬆ ファイルをアップロード」の 3 つから選択
- 「カメラで撮影」はブラウザの `getUserMedia` でその場のライブ撮影、「写真ライブラリ」は端末の写真から選択（`accept="image/*"`）、「ファイル」は一眼レフから取り込んだ画像ファイル等を選択
- ※ スマホでは OS 側の選択シート仕様により「写真ライブラリ」と「ファイル」の表示が統合される場合があります
