...
- マクロ術前/術後・ミクロ術前/術後の各スロットの「＋ 画像を追加」を押すと、端末標準の選択メニュー（写真を撮る／写真ライブラリ／ファイルを選択）が直接開きます
- カメラ・写真ライブラリ・ファイルはすべて OS 標準メニューに委譲しているため、アプリ側の独自ポップアップは表示されません（メニューの二重表示を避けるため）
