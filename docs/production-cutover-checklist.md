...
| `20260715000017_admin_unlimited_user_invites.sql` | 2026-07-15 適用 | `admin_unlimited_user_invites` | 2026-07-15 | **機能**: `create_user_invite()` で admin（`is_admin()` = developer/admin）を「有効な未使用コード最大5件」上限から免除し、無制限に発行できるように変更。一般ユーザーは従来どおり5件上限を維持（`if not public.is_admin() and v_active >= 5`）。発行内容（role=dentist / invitee=10 / inviter=0 / max_uses=1）・RLS・execute grant は不変。admin であっても本RPC経由は dentist 固定で developer 昇格コードは不可。本番適用済み・function 定義で免除ロジックを実測確認。|
