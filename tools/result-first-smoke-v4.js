...
(async()=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const t=performance.now();const ok=window.cfOpenResultScreen();const returned=performance.now()-t;
    await new Promise(r=>setTimeout(r,100));
    const shell=document.getElementById('cfFastResultShell'),compare=document.getElementById('cfFastCompare'),range=document.getElementById('cfFastRange');
    range.value='72';range.dispatchEvent(new Event('input',{bubbles:true}));
    return {ok,returned,open:!!shell&&shell.classList.contains('open'),aria:shell&&shell.getAttribute('aria-hidden'),before:(document.getElementById('cfFastBefore')||{}).src?.length||0,after:(document.getElementById('cfFastAfter')||{}).src?.length||0,split:compare&&compare.style.getPropertyValue('--cf-fast-split'),legacyActive:document.getElementById('result').classList.contains('active'),wait:!!document.querySelector('.cfWaitOverlay.show,.cfWaitOverlay.active'),v9:!!(window.go&&window.go.__cfFastResultV9),opened:Number(window.__cfFastResultOpenedAt||0),painted:Number(window.__cfFastResultPaintedAt||0)};
  });
  console.log(JSON.stringify({result,errors},null,2));
  assert(result.ok,'fast result route returned false');assert(result.returned<300,'fast result route did not return immediately');assert(result.open,'fast result shell did not open');assert.strictEqual(result.aria,'false');assert(result.before>100&&result.after>100,'images were not painted');assert.strictEqual(result.split,'72%');assert.strictEqual(result.legacyActive,false,'heavy legacy result screen activated');assert.strictEqual(result.wait,false,'waiting overlay remained');assert(result.v9,'v9 route guard missing');assert(result.opened>0&&result.painted>0,'timing markers missing');
