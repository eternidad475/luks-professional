...
(async()=>{
  ...
  const result=await Promise.race([
    page.evaluate(()=>{
      ...
      const rawRaf=window.requestAnimationFrame;let captured=0;
      ...
      }finally{window.requestAnimationFrame=rawRaf;}
    }),
    new Promise((_,reject)=>setTimeout(()=>reject(new Error('result-first evaluation exceeded 8 seconds')),8000))
  ]);

  console.log(JSON.stringify({result,pageErrors},null,2));
  assert.strictEqual(result.ok,true,'result painter returned false');
  ...
  assert.strictEqual(result.gridChildren,0,'edit grid hydrated during first paint');
  assert(result.editHydrationScheduled>0,'deferred hydration was not scheduled');
